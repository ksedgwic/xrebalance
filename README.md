...
| its next plan only retry routes that already failed for liquidity in this request, within a fragment of the failed amounts | ends the request unsent — a liquidity failure rules out the amount tried, not the route, so the next plan can be the same route one msat lower, round after round     |
...
| a forwarder refuse the HTLC's expiry as beyond its locktime limit (`expiry_too_far`), a limit gossip does not carry        | temporarily excludes the channel                                                                                                                                        |
